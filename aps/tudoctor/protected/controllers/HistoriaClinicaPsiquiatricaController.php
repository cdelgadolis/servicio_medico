...
class HistoriaClinicaPsiquiatricaController extends Controller
{
/**
...
{
$model=new HistoriaClinicaPsiquiatrica;
...
if(isset($_POST['HistoriaClinicaPsiquiatrica']))
{
$model->attributes=$_POST['HistoriaClinicaPsiquiatrica'];
$model->paciente=$paciente->id_paciente;
$model->fk_usuario_creacion=Yii::app()->user->id;
if($model->save())
$this->redirect(array('view','id'=>$model->id_hc_psiquiatrica));
}
...
public function actionBuscarPaciente()
{
		...
		if (isset($_POST["BuscarPaciente"]))
	{
		...
		if($existe == true) {
			$consulta2 = "SELECT id_hc_psiquiatrica FROM historia_clinica_psiquiatrica WHERE paciente='".$id."' ";
			...
			foreach($Filas as $Fila)
				...
				$this->redirect(array('historiaClinicaPsiquiatrica/create', 'id'=>$id));
				}
...
{
$dataProvider=new CActiveDataProvider('HistoriaClinicaPsiquiatrica');
...
{
$model=new HistoriaClinicaPsiquiatrica('search');
$model->unsetAttributes();  // clear any default values
if(isset($_GET['HistoriaClinicaPsiquiatrica']))
$model->attributes=$_GET['HistoriaClinicaPsiquiatrica'];
...
{
$model=HistoriaClinicaPsiquiatrica::model()->findByPk($id);
if($model===null)
...
{
if(isset($_POST['ajax']) && $_POST['ajax']==='historia-clinica-psiquiatrica-form')
{
